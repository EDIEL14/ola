                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    // Si se ha definido un item, procede a insertar
                    if (isset($nombre_item)) {
                        $sql = "INSERT INTO Historial_Compras (tipo_item, nombre_item, precio, metodo_pago, fecha_hora, email_usuario, id_usuario) 
                                VALUES (?, ?, ?, ?, ?, ?, ?)";
                        $stmt = $conn->prepare($sql);
                        $stmt->bind_param("ssdsssi", $tipo_item, $nombre_item, $precio, $metodo_pago, $fecha_hora, $email_usuario, $id_usuario);
